...
public class Tournament {
	...
	public Game createGame(final Team homeTeam, final Team vistingTeam) {
		if (homeTeam == null || vistingTeam == null) {
			throw new IllegalArgumentException("Team must exist");
		}else if (homeTeam == vistingTeam) {
			throw new IllegalArgumentException("Teams must be distinct");
		}else if (!teams.values().contains(vistingTeam) || !teams.values().contains(homeTeam)) {
			throw new IllegalArgumentException("Teams must be registered in the same tournament");
		}
	...
	public void createGamePlan() {
		final List<Team> teamList = this.getTeams();
		for(int i = 0; i< teamList.size(); i++){
			for(int j= i; j < teamList.size(); j++){
				this.createGame(teamList.get(i), teamList.get(j));
			}
		}
	}

	public void generateGameResults() {
		final int MAX_GOALS = 8;
		Random random = new Random();
		for(Game game: this.getGames()){
			game.storeResult(random.nextInt(MAX_GOALS), random.nextInt(MAX_GOALS));
		}
	}
}
